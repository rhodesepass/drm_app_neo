...
static int join_path(char *dst, size_t dst_sz, const char *base, const char *rel) {
...
static int file_exists_executable(const char *filepath) {
    if (!filepath || filepath[0] == '\0') return 0;
    return access(filepath, X_OK) == 0;
}

static void set_lvgl_path(char *dst, size_t dst_sz, const char *abs_path) {
    if (!dst || dst_sz == 0) return;
