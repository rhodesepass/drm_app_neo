...
// 用户选中一项即回填 uix_session 并回主菜单；不选则由 UIX 超时收屏。
//
...
lv_obj_t *screen_usbselect_create(void)
{
    ...
    self.btn[UIX_USB_CHOICE_MTP]         = ui_text_button(root, 28, 70, 149, 51, UI_SEM_NEUTRAL, "文件传输", on_mtp);
    self.btn[UIX_USB_CHOICE_EPASS]       = ui_text_button(root, 187, 70, 147, 51, UI_SEM_NEUTRAL, "ePass管理", on_epass);
    self.btn[UIX_USB_CHOICE_FIDO]        = ui_text_button(root, 28, 127, 149, 51, UI_SEM_NEUTRAL, "FIDO密钥", on_fido);
    self.btn[UIX_USB_CHOICE_CHARGE_ONLY] = ui_text_button(root, 187, 127, 147, 51, UI_SEM_NEUTRAL, "仅充电", on_charge);
