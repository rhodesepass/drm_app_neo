...
### 5) PC 仿真（sim/ 的 overlay 仿真 target）

真实的 overlay 代码（元素引擎 / transitions / worker / prts_timer / layer_animation
全链路）可以在 PC 上跑：drm_warpper 换成 `sim/sim_drm_warpper.c` 的 mock（malloc
buffer + 层状态记录），SDL 合成器按层坐标/alpha 混叠出画面（等价 DEBE）；video 层
用**编译期传入的固定图片**替代 Cedar 解码。

```bash
# 依赖：sdl2 freetype2 libpng libdrm(仅头文件)
cmake -B sim/build sim               # -DSIM_VIDEO_IMG=/path/to.jpg 换视频底图
cmake --build sim/build -j
./sim/build/epass_overlay_sim_360x640   # 或 720x1280
```

- 按键：`1..5` 切干员预设（`sim/overlay_sim_presets.c`，覆盖 arknights 默认/自定义
  文字、custom 文本特效、custom 运动几何、image 五类）；`0`/ESC 仅 abort（看滑出）；
  `f/m/w` = fade/move/swipe 过渡；`q` 退出
- 无头截图（CI/无显示环境）：
  ```bash
  SDL_VIDEODRIVER=dummy SIM_PRESET=3 SIM_SHOT=/tmp/shot.bmp SIM_SHOT_MS=2500 \
      ./sim/build/epass_overlay_sim_360x640
  ```
  `SIM_SHOT_MS` 从进程启动计时，多跑几个时间点即可核对动画各阶段。
- 已知差异：无 vsync/扫描语义（合成器 ~60fps 轮询读 buffer，撕裂窗口与真机不同但同类）；
  干员预设是 C 构造的元素列表，**不覆盖 operators.c 的 epconfig.json 解析路径**；
  切预设用 `overlay_abort` 直驱，不经过 PRTS 排期状态机。
