...
opinfo 有两个绘制后端（都在 `src/overlay/opinfo.c`）：

- `arknights` → 专用实现（`overlay_opinfo_show_arknights`，手写 worker 状态机，
  模板/坐标/帧数直接对 `config.h` 的 `OVERLAY_ARKNIGHTS_*` / `OVERLAY_ANIMATION_OPINFO_*` 宏）
- `image` / `custom` → **元素引擎**：解析阶段翻译成"元素列表"（`olopinfo_element_t[]`），
  由同一个 worker 状态机驱动：
...
- **生命周期**：引擎控制块是 static（timer 回调线程与 worker 线程都会引用它，而
  ...
  含 scroll 元素的列表永不自然结束，跑到 abort 为止。
...
- `anchor`（缺省 `tl`）：`tl/tr/bl/br`，右/下锚定时 `x/y` 是元素相应边到屏幕边缘的距离，
  让"贴右下角"不依赖图片尺寸（如 `anchor=br, x=10, y=10` 贴右下角 logo）
...
- 只影响 custom/image；arknights 走专用实现，与引擎能力无关
...
是"需要 worker 的效果"的完整参考实现；同文件里 arknights 的专用实现
（`arknights_overlay_worker` 一族）是手写单效果 worker 的参考。